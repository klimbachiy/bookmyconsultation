...
public class DoctorService {
	...
	public Doctor register(Doctor doctor) throws InvalidInputException {
		...
		if(null == doctor.getAddress()){
			throw new InvalidInputException(Collections.singletonList("Invalid Address"));
		}else{
			doctor.setId(UUID.randomUUID().toString());
			if(null == doctor.getSpeciality()){
				doctor.setSpeciality(Speciality.GENERAL_PHYSICIAN);
			}
			Address docAddress = doctor.getAddress();
			Address savedAddress = addressRepository.save(docAddress);
